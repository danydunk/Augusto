...
public class AlloyUtil {
	 ...
	 * @return
	 */
	public static Signature searchSignatureInList(final List<Signature> signatures, final String label) {
	...
	public static AlloyEntity searchElementInList(final List<? extends AlloyEntity> elements, final String label) {
	...
	static public Module compileAlloyModel(final File model) throws Exception {
		...
		try {
			final Module out = CompUtil.parseEverything_fromFile(rep, null, model.getAbsolutePath());
			return out;
		} catch (final Exception e) {
			e.printStackTrace();
			throw new Exception("AlloyUtil - compileAlloyModel: compilation error.");
	 ...
	 * @throws Exception
	 */
	static public A4Solution runCommand(final Module model, final Command run_command) throws Exception {
		...
		try {
			solution = TranslateAlloyToKodkod.execute_command(rep, model.getAllReachableSigs(), run_command, options);
		} catch (final Exception e) {
	 ...
	 * @throws Exception
	 */
	static public A4Solution runCommand(final Module model, final Command run_command, final long timeout)
			throws Exception {
		...
		final class Run_command_thread extends Thread {
			...
			public void run() {
				...
				options.solver = A4Options.SatSolver.SAT4J;
				try {
					final A4Solution app = TranslateAlloyToKodkod.execute_command(rep, model.getAllReachableSigs(),
							run_command, options);
					this.solution = app;
				} catch (final Err e) {
					this.exception = true;
	...
	static public Alloy_Model loadAlloyModelFromFile(final File... files) throws Exception {
		...
		final Alloy_Model model = new Alloy_Model(signatures, facts, predicates, functions, open_statements);
		return model;
	}
	...
	static public Alloy_Model loadAlloyModelFromString(final String model) throws Exception {
		...
		final Alloy_Model out = new Alloy_Model(signatures, facts, predicates, functions, open_statements);
		return out;
	}

	private static List<Function> retrieveFunctions(final Module alloyFromFile, final List<Signature> signatures,
		...
		if (!lines[pos].trim().endsWith("{}") && !lines[pos].trim().endsWith("{ }")) {
			...
			for (int x = pos + 1; x < lines.length; x++) {
				...
				if (lines[x].trim().startsWith("pred") || lines[x].trim().startsWith("abstract")
						...
						|| lines[x].trim().startsWith("fact") || lines[x].trim().startsWith("assert")
						|| lines[x].trim().startsWith("run") || lines[x].trim().startsWith("one sig")) {
					System.out.println();
	...
	private static List<Predicate> retrievePredicates(final Module alloyFromFile, final List<Signature> signatures,
			final String source) throws Exception {
		...
		for (final Func func : alloyFromFile.getAllFunc()) {
			if (func.isPred && !func.label.contains("run$")) {
				...
				// we get the content of the predicate from the source code
				final String content = getBody(lines, pos);
				final Predicate predicate = new Predicate(func.label.replaceAll("this/", ""), content,
						getDeclarationParameters(func, signatures));
	...
	private static LinkedHashMap<Signature, List<String>> getDeclarationParameters(final Func func,
		...
		for (final Decl par : pars) {
			final Signature sig = searchSignatureInList(signatures, par.expr.toString().replace("this/", ""));
	...
	private static ArrayList<Fact> retrieveFacts(final Module alloyFromFile, final String source) throws Exception {
	...
	private static ArrayList<Signature> retrieveSignatures(final Module alloyFromFile) throws Exception {
		...
		while (true) {
			...
			loop: for (final Sig signature : sigs) {
				...
				if (signature instanceof PrimSig) {
					...
					if (sig.parent != null && sig.parent != PrimSig.UNIV && sig.parent != PrimSig.NONE) {
					...
					if (sig.parents == null || sig.parents.size() == 0) {
						throw new Exception("AlloyUtil - retrieveSignatures: error parsing subset signature.");
					}
					...
					for (final Sig par_sig : sig.parents) {
						...
						final Signature par = searchSignatureInList(signatures, par_sig.label.replaceAll("this/", ""));
						parents.add(par);
						if (par == null) {
							to_process.add(signature);
							continue loop;
		...
		for (final Sig signature : sigFromModule) {
			for (final Field field : signature.getFields()) {
				final Signature ourSig = searchSignatureInList(signatures, signature.label.replaceAll("this/", ""));
				...
				if (field.decl().expr instanceof ExprUnary) {
					...
					if (ourSig == null || co_dom == null) {
						throw new Exception("AlloyUtil - retrieveSignatures: error parsing relations.");
					}
					ourSig.addBinaryRelation(field.label.replaceAll("this/", ""), co_dom, getCardinality(unary.op));
					...
					ourSig.addTernaryRelation(field.label.replaceAll("this/", ""), co_dom, mid_dom, cards[0], cards[1]);
				}
			}
		}
		return signatures;
