package usi.guisemantic.semantic;
...
public class AlloyUtilsTest {
	...
	public void testParserFileAssertSignatures() throws Exception {
		...
		final Alloy_Model model = AlloyUtil.loadAlloyModelFromFile(new File("./resources/for_test/alloy/CRUD.als"));
		assertNotNull(model);
	...
	public void testComplete() throws Exception {
		...
		final edu.mit.csail.sdg.alloy4compiler.ast.Module mAlloy = AlloyUtil.compileAlloyModel(modelString);
		assertNotNull(mAlloy);
		assertEquals(mAlloy.getAllSigs().size(), model.getSignatures().size());

	}

	private static Signature searchSignatureInList(final List<Signature> signatures, final String label) {
	...
	private static AlloyEntity searchElementInList(final List<? extends AlloyEntity> elements, final String label) {
